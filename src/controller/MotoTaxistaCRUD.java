...
public class MotoTaxistaCRUD extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		/*
		 * else if (cmd.equalsIgnoreCase("chamados")) { int cod =
		 * Integer.parseInt(request.getParameter("cod"));
		 * 
		 * try { Empresa empresa = empresaServico.buscar(cod);
		 * request.setAttribute("cod", empresa.getCodEmpresa()); forward =
		 * FORM_EMPRESA; } catch (RuntimeException e) {
		 * request.setAttribute("erro", "Erro de execução: " + e.getMessage());
		 * forward = ERRO; } RequestDispatcher rd =
		 * request.getRequestDispatcher(forward); rd.forward(request, response);
		 * }
		 */
